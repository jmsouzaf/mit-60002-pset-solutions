...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        num_tiles = len(self.tiles) - len(self.furniture_tiles)
        return num_tiles
    ...
    def get_random_position(self):
        ...
        """
        rand_pos = Position(self.width * random.random(), self.height * random.random())
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        new_pos = self.position.get_new_position(450 - self.direction, self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    # Keep track of each time-step total for each trial:
    all_time_steps = []

    for i in range(num_trials):
        # Create room and robots:
        room = EmptyRoom(width, height, dirt_amount)
        all_robots = [robot_type(room, speed, capacity) for _ in range(num_robots)]

        time_steps = 0
        # Loop until room reaches minimum coverage:
    ...
    mean_time_steps = sum(all_time_steps) / len(all_time_steps)
